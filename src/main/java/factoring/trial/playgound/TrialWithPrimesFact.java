...
public class TrialWithPrimesFact implements FactorizationOfLongs {
	...
	public TrialWithPrimesFact(int maxFactor) {
	...
	public long findFactors(long n, Collection<Long> primeFactors) {
		for (int primeIndex = 0 ; prime[primeIndex] < maxFactor; primeIndex++) {
			//            if (maxFactorIndex == 6542)
			//                System.out.println();
			while (n%prime[primeIndex] == 0 && n > 1) {
				//				if (primeFactors == null)
				//					return prime[primeIndex];
