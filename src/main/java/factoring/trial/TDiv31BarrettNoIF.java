...
//			found = 1;
	...
	public ExponentsMaxFactor fillFactorCounts(int N, int maxIndex) {
		...
		while ((number & 1) == 0){
			number >>= 1;
			exponent++;
			// TODO remove this; if we have a square outside of the factor base we can still use it, but we need to
			// keep track of this Numbers outside of the factor base and handle them like large primes in the QS
			maxFactorIndex = 1;
		}
		if ((exponent & 1l) == 1l ) {
			factors.set(1);
