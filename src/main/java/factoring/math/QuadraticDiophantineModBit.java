...
public class QuadraticDiophantineModBit {
	...
	public byte[][] lehmanSolutions() {
		final byte[][] diffToSolution = new byte[mod][mod];

		for (int kn=1; kn< mod ; kn+= 1) {
			//			if (kn % 3 != 0)
			{
				...
				final byte[] currentSol = diffToSolution[kn/2];
				for (int a = 0; a < mod; a++) {
				...
				final String diffString = IntStream.range(0, currentSol.length).mapToObj(i -> Integer.toString(i) + "->" + currentSol[i]).collect(Collectors.joining(", "));
