...
public class Row {
    ...
    public boolean xor(Row col, int columnIndex) {
        if (!entries.get(columnIndex))
            System.out.println();
        if (!col.entries.get(columnIndex))
            System.out.println();
        this.entries.xor(col.entries);
        this.columns.xor(col.columns);
        return this.entries.get(columnIndex);
    }
    ...
    public String toString(List<Pair<Multiset<Integer>, Multiset<Integer>>> factors, long n) {
        ...
        prodRight = prodRight < 0 ? prodRight + n : prodRight;
        int xPlusY = (int) ((prodLeft + prodRight) % n);
        int xMinusY = (int) ((prodLeft + (n -prodRight)) %n);
