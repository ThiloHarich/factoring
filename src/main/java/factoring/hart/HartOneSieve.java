 ...
 */
public class HartOneSieve extends FactorAlgorithm {
	private static final Logger LOG = Logger.getLogger(HartOneSieve.class);
	 ...
	 */
	public HartOneSieve(boolean doTDivFirst) {
		this.doTDivFirst = doTDivFirst;
		// Precompute sqrts for all k < I_MAX
		sqrts = new double[I_MAX];
		ks = new int[I_MAX];
		int k =1;
		...
		for (int i=1; i<I_MAX/2; i++) {
			...
			{
				ks[k] = i*K_MULT1;
				sqrts[k] = Math.sqrt(i * (double)K_MULT1);
				k++;
			}
			if (i%7 != 0 && i%9 != 0) {
				ks[k] = i * K_MULT2;
				sqrts[k] = Math.sqrt(i * (double)K_MULT2);
				k++;
			}
		}
	}
	...
	public long findSingleFactor(long n) {
		...
		for (int i=1; ;i++) {
			...
			final int k = ks[i];
			long a = USE_FUSED_MULTIPLY_ADD ? (long)  Math.fma(sqrt4N, sqrts[i], ROUND_UP_DOUBLE) : (long) (sqrt4N * sqrts[i] + ROUND_UP_DOUBLE);
			a = adjustA(n, a, k);
