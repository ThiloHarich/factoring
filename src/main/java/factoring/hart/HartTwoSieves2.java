 ...
 */
public class HartTwoSieves2 extends FactorAlgorithm {
	private static final Logger LOG = Logger.getLogger(HartTwoSieves2.class);
	...
	private final double[] sqrt3 = new double[I_MAX];
	private final int [] k1s = new int[I_MAX];
	private final int [] k2s = new int[I_MAX];
	private final int [] k3s = new int[I_MAX];
	 ...
	 */
	public HartTwoSieves2(boolean doTDivFirst) {
		...
		for (int i=1; i<I_MAX; i++) {
			...
			if ((2*i-1) % 9 != 0) {
				k1s[k] = (2*i-1)*K_MULT1;
				sqrt1[k] = Math.sqrt((2*i-1)*K_MULT1);
				k++;
			}
			if ((2*i) % 9 != 0) {
				k3s[l] = (2*i)*K_MULT1;
				sqrt3[l] = Math.sqrt((2*i)*K_MULT1);
				l++;
			}
	...
	public long findSingleFactor(long n) {
		...
		for (int i=1; ;i++) {
			...
			final int k3 = k3s[i];
			a = USE_FUSED_MULTIPLY_ADD ? (long)  Math.fma(sqrt4N, sqrt3[i], ROUND_UP_DOUBLE) : (long) (sqrt4N * sqrt3[i] + ROUND_UP_DOUBLE);
			a = adjustA(n, a, k3);
			test = a*a - k3 * fourN;
