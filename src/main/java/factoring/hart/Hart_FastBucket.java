 ...
 */
public class Hart_FastBucket extends FactorAlgorithm {
	private static final Logger LOG = Logger.getLogger(Hart_FastBucket.class);
	...
	private final int bucketsPow2 = 3;

	private final long buckets = (1 << bucketsPow2);
	private final long bucketsMin1 = buckets -1;
	 ...
	 */
	public Hart_FastBucket(boolean doTDivFirst) {
		...
		int bucketsFilled = 0;
		for (int i=0; i <= bucketsMin1; i++) {
			for (final int j=0; i <= bucketsMin1 && bucketsFilled < buckets; i++) {
				final int index = (int) ((i/(double)j* buckets));
	...
	public long findSingleFactor(long N) {
		...
		try {
			for (int i=1; ;i++, k += K_MULT) {
				...
				b = (long) Math.sqrt(test << (bucketsPow2  * 2));
				final int bucket = (int) (b & bucketsMin1) ;
				final int k2 = k * multiplier[bucket];
	...
	public static void main(String[] args) {
		...
		final Hart_FastBucket holf = new Hart_FastBucket(false);
