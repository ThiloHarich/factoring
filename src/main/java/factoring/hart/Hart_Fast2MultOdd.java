 ...
 */
public class Hart_Fast2MultOdd extends FactorAlgorithm {
	private static final Logger LOG = Logger.getLogger(Hart_Fast2MultOdd.class);
	 ...
	 */
	public Hart_Fast2MultOdd(boolean doTDivFirst) {
	...
	public long findSingleFactor(long N) {
		...
		final int cbrt = (int) (Math.cbrt(N) / 30) * 3;
		long a, b, test, gcd;

		try {
			final int step = 3;
			for (int i=3, k1 = i * K_MULT1, k2 = i * K_MULT2; i < cbrt ;i+=step, k1 += step*K_MULT1, k2 += step*K_MULT2) {
				//				for (int i=2, k1 = i * K_MULT1, k2 = i * K_MULT2; i < cbrt;i+=2, k1 += 2*K_MULT1, k2 += 2*K_MULT2) {
				// using the fusedMultiplyAdd operation defined in IEEE 754-2008 gives ~ 4% speedup
				...
				// avoid that we hit the same values as in the first 315 case
			...
			for (int i=2, k1 = i * K_MULT1, k2 = i * K_MULT2; i < cbrt ;i+=step, k1 += step*K_MULT1, k2 += step*K_MULT2) {
				// using the fusedMultiplyAdd operation defined in IEEE 754-2008 gives ~ 4% speedup
				a = IEEE_754_2008 ? (long)  Math.fma(sqrt4N, sqrt1[i], ROUND_UP_DOUBLE) : (long) (sqrt4N * sqrt1[i] + ROUND_UP_DOUBLE);
				a = adjustA(N, a, k1);
				test = a*a - k1 * fourN;
			...
			for (int i=1, k1 = i * K_MULT1, k2 = i * K_MULT2; i < cbrt ;i+=step, k1 += step*K_MULT1, k2 += step*K_MULT2) {
				// using the fusedMultiplyAdd operation defined in IEEE 754-2008 gives ~ 4% speedup
				a = IEEE_754_2008 ? (long)  Math.fma(sqrt4N, sqrt1[i], ROUND_UP_DOUBLE) : (long) (sqrt4N * sqrt1[i] + ROUND_UP_DOUBLE);
				a = adjustA(N, a, k1);
				test = a*a - k1 * fourN;
			...
			for (int i=cbrt, k1 = i * K_MULT1, k2 = i * K_MULT2; ;i++, k1 += K_MULT1, k2 += K_MULT2) {
				// using the fusedMultiplyAdd operation defined in IEEE 754-2008 gives ~ 4% speedup
				a = IEEE_754_2008 ? (long)  Math.fma(sqrt4N, sqrt1[i], ROUND_UP_DOUBLE) : (long) (sqrt4N * sqrt1[i] + ROUND_UP_DOUBLE);
				a = adjustA(N, a, k1);
				test = a*a - k1 * fourN;
