 ...
 */
public class Hart_FastT6 extends FactorAlgorithm {
	private static final Logger LOG = Logger.getLogger(Hart_FastT6.class);
	 ...
	 */
	public Hart_FastT6(boolean doTDivFirst) {
	...
	public long findSingleFactor(long N) {
		...
		long a, b, test, gcd, oldA = 0, aOrig;
		int k = K_MULT;
		try {
			for (int i=1; ;i++, k += K_MULT) {
				...
				if (b*b == test) {
					final long aDiff = a - oldA;
					final double aDiffApprox = Math.sqrt(N / (double)k)*K_MULT;
					System.out.println(aDiffApprox / aDiff);
