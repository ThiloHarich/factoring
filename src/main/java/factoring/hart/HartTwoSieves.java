 ...
 */
public class HartTwoSieves extends FactorAlgorithm {
	 ...
	 */
	private static final int I_MAX = 1<<22;
	 ...
	 */
	public HartTwoSieves(boolean doTDivFirst) {
		this.doTDivFirst = doTDivFirst;
		// Precompute sqrts for all k < I_MAX

		for (int i=0, k =1, j =1; k<I_MAX; i++) {
...
//			long a = USE_FUSED_MULTIPLY_ADD ? (long)  Math.fma(sqrt4N, sqrt1[i], ROUND_UP_DOUBLE) : (long) (sqrt4N * sqrt1[i] + ROUND_UP_DOUBLE);
			long a = (long) (sqrt4N * sqrt1[i] + ROUND_UP_DOUBLE);
			a = adjustA(n, a, k1);
