 ...
 * It is an extension of the {@link LehmanFactorFinder}
 * The features different from LehmanFactorFinder} are:
 * 1) Move the cross point when to switch from trial division to the fermat factoring towards trial division.
 * 2) First use numbers for k (in x^2 - 4kn = y^2) with a factor 3.
 *
 * 1) increases the numbers to be tested per loop and such increase the performance
 * 2) investigates the fermat equation modulo 3. Together with the arguments modulus 4 as used in LehmanFactorFinder
 * this can be seen as an argument modulus 12, which gives the name.
 *
 * Both features work best if the number to be factorized has big factors.
 *
 ...
 * But the range check for one k involves the expensive sqrt calculations for lookups, preventing it
 ...
 * Beside the arguments modulus 2 and 4 (lehmam) and 32 (LehmanFactorFinder) for calculating the step for x
 ...
 * While Lehman suggests to first use numbers which have a small smooth factor like 12 = 2*2*3.
...
//            int xStepMultiplier = 1;
            for (int k = kBegin; k <= kMax; k+=3) {
                ...
                float xRange = nPow1Sixth * sqrtInv[k];
