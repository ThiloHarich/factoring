 ...
 */
public class LehmanMult extends FactorAlgorithm {
	...
	public long findSingleFactor(long N) {
		...
		if ((factor = lehmanKEven (kTwoA, (kLimit /6) * 6, trialPhase)) > 1 && factor < N)
			return factor;
		...
		if ((factor = lehmanKEven ((kLimit /6) * 6, kLimit << 1, 0)) > 1 && factor < N) {
			foundInStep[9]++;
			return factor;
		}
