...
=======
	static double oneThird = 1.0/3;
	static float BALANCE_TRIAL = 1.3f;
	static float BALANCE_TRIAL_CUBE = BALANCE_TRIAL * BALANCE_TRIAL;
	static int K_MAX = (int) (Math.ceil(Math.pow(Long.MAX_VALUE, oneThird)) / BALANCE_TRIAL_CUBE);
...
=======
		double maxTrialFactor =  Math.ceil(BALANCE_TRIAL * Math.pow(n, oneThird));
>>>>>>> d9917b1 some changes
...
=======
		maxTrialFactor =  BALANCE_TRIAL * Math.pow(n, oneThird);
		final int kMax = (int) (Math.ceil(maxTrialFactor / BALANCE_TRIAL_CUBE));
>>>>>>> d9917b1 some changes
...
=======
				xStep = 4;
				//				xBegin = (xBegin & xBeginMod4) | (k + nMod4) % 4;
				xBegin = xBegin + (k + nMod4 - xBegin) % 4;
				//				xBegin |= (k + nMod4) % 4;
				if (xBegin < sqrt4kn)
					xBegin += 4;
>>>>>>> d9917b1 some changes
