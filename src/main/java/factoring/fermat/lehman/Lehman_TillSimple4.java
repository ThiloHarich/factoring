...
public class Lehman_TillSimple4 extends FactorAlgorithmBase {
	...
	public Lehman_TillSimple4(float tDivLimitMultiplier) {
	...
	public long findSingleFactor(long N) {
		...
		// make it odd
		final int range4 = Math.max(((kLimit >> 8) - 1), 0) | 1;
		final int range2 = Math.max(((kLimit >> 6) - 1), 0) | 1;
		final long fourN = N<<2;
		final double sqrt4N = Math.sqrt(fourN);
		final double sixthRootTerm = 0.25 * Math.pow(N, 1/6.0); // double precision is required for stability
		int kSmall=1;
		for (; kSmall <= range4; kSmall++) {
		...
		for (int kEven = kSmall; kEven <= kLimit; kEven+=2) {
			long a = (long) (sqrt4N * sqrt[kEven] + ROUND_UP_DOUBLE) | 1;
			long test = a*a - kEven * fourN;
