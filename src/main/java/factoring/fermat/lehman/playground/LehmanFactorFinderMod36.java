 ...
 * a (small?) k < kMax < = n^1/3 * n^4/3 = n^(1/4 + 1/3) = n^(3/12 + 4/12) = n ^ 7/12
 * and investigate in numbers
 * x^2 - 4kn = y^2
 * d.h. for k=1 -> xRange = n^1/3 * n^3/4 =
 *
 * Mod 15:
 * 15, 10, 5               -5
 * 14, 9, 4  -> 9,4,14     -5
 ...
 * 7^2 = 4 mod 9
 * 8^2 = 1 mod 9
 * kn = 1 -> x^2 = y^2 + 4 -> y^2 = 0, x^2 = 4 -> x=2,7
 * kn = 2 -> x^2 = y^2 + 8 -> x=0,3,6
 * kn = 4 -> x^2 = y^2 + 7 -> y^2 = 0, x^2 = 7 -> x=4,5
 * kn = 7 -> x^2 = y^2 + 1 -> y^2 = 0, x^2 = 7 -> x=1,8
 *
 ...
 */
public class LehmanFactorFinderMod36 implements FactorizationOfLongs {

	static double ONE_THIRD = 1.0 / 3;
	static double ONE_NINTH = 1.0 / 9;
	 ...
	 */
	public LehmanFactorFinderMod36(int bits, float maxFactorMultiplierIn) {
		if (bits > 41)
	...
	public long findFactors(long n, Collection<Long> primeFactors) {
		...
		final long n4 = 4 * n;
		final byte n4Mod9 = (byte) (n4 % 9);
		final byte nMod9 = (byte) (n % 9);
		...
		//        for a given n determine the k which results in kn mod 9 = 1,4 or 7
		final int [] ksMod9 = {nInvMod9,(4*nInvMod9)%9,(7*nInvMod9)%9};
		//        start with x = xBegin + (xBegin-2 % 9)
		//        while (x%2==0) x += 9;
		//        for mod = 2*9 calculate all residue with x = 1 mod 2 and x = 2,7 (4,5 and 1,8) mod 9
		//        x = 2 mod 9 if x mod 2^k != r , x+2=4,x+4=6,x+8=1,x+10=3,x+14=7
		for (int i=0; i <3; i++) {
			final int kBegin = ksMod9[i];
			assertEquals(kBegin * n % 9, 3 * i + 1);
			for (int k = kBegin; k <= kMax; k += 9) {
				final double sqrt4kn = sqrt[k] * sqrt4N;
				final int xBeginOrig = (int) (sqrt4kn + ROUND_UP_DOUBLE);
				final float xRange = nPow1Sixth * sqrtInv[k];
				final int xEnd = (int) (sqrt4kn + xRange);
				for (int j=0; j<2; j++) {
					// kn = 1 mod 9 -> x = 2,7 mod 9
					// kn = 4 mod 9 -> x = 4,5 mod 9
					// kn = 7 mod 9 -> x = 8,1 mod 9
					int xBegin = xBeginOrig;
					//                    xBegin += j==0 ? mod9((2 << i) - xBeginOrig) : mod9(9 - xBeginOrig - (2 << i));
					xBegin += j==0 ? PrimeMath.mod((2 << i) - xBeginOrig, 9) : PrimeMath.mod(9 - xBeginOrig - (2 << i), 9);
					if (j==0)
						assertEquals((2 << i) , xBegin %9);
					else
						assertEquals(9 - (2 << i) , xBegin %9);
					int xStep = 2*9;
					if (k % 2 == 0) {
						while (xBegin < xEnd && xBegin % 2 == 0)
							xBegin += 9;
					} else {
						//                    if (k * n % 4 == 3) {
						//                        xStep = 8;
						...
						//                    } else {
						xStep = 4*9;
						final int kPlusNMod4 =  ((k + (int)n) % 4);
						while (xBegin < xEnd && xBegin % 4 != kPlusNMod4)
							xBegin += 9;
						//                    xBegin = xBegin + PrimeMath.mod(k + nMod4 - xBegin, 4);
						//                    }
					}
					...
					for (long x = xBegin; x <= xEnd; x += xStep) {
						//                    while (xBegin % 9 != 3*i +1 && xBegin % 9 != 8 - (3*i))
						//                        xBegin += xStep;
						// here we start using long values
						// in java the trick to replace the multiplication with an addition does not help
						final long x2 = x * x;
	...
	private int mod9(double x) {
		final int mod9 = (int) (x - 9 * ((int) (x * ONE_NINTH) - 1));
		return mod9;
	}
	...
	public String toString() {
		return "LehmanFactorFinderMod36{" +
