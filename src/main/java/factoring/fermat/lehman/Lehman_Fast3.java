 ...
 */
public class Lehman_Fast3 extends FactorAlgorithmBase {
	private static final Logger LOG = Logger.getLogger(Lehman_Fast3.class);

	/** This is a constant that is below 1 for rounding up double values to long. */
	private static final double ROUND_UP_DOUBLE = 0.9999999665;

	private static double[] sqrt, sqrtInv, sqrt6, sqrt3;
	...
	static {
		...
		final int kMax = 1<<22;
		sqrt = new double[kMax >> 6 + 1];
		sqrtInv = new double[kMax >> 6 + 1];
		sqrt6 = new double[kMax/6 + 1];
		sqrt3 = new double[kMax/6 + 1];
		for (int i = 1; i < sqrt6.length; i++) {
	 ...
	 */
	public Lehman_Fast3(boolean factorSemiprimes) {
	...
	public long findSingleFactor(long N) {
		...
		if ((factor = lehmanOdd(kTwoA, kLimit6)) > 1)
			return factor;
	...
	private long lehmanOdd(int kBeginIndex, final int kLimit) {
		int kIndex = kBeginIndex;
		for (int k = 6*kBeginIndex + 3; k <= kLimit; k += 6) {
			long a = (long) (sqrt4N * sqrt3[kIndex] + ROUND_UP_DOUBLE);
	...
	public static void main(String[] args) {
		...
		final Lehman_Fast3 lehman = new Lehman_Fast3(true);
