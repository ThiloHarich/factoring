...
public class LehmanLongFactorFinder implements SingleLongFactorFinder {
     ...
     * For the small numbers we use a trial division which does the trial division only by primes -> only running time n^1/3 / log(n).
     * Lehman takes n^1/3. So the cross over point should be at c * n^1/3, where c > 1. When choosing c > 1 the maximal k in the lehman
     * algorithm is cut down by c^2. This is also nice, since for high k not every k results in a candidate for the lehman argument, but
     * we have to check some preconditions to find this out. So a c > 1 really speeds up the algorithm here.
     * TODO the number of numbers to be considered (for k = 1) is multiplied by c^3. There are algorithms (using some modulo arguments)
     * which work much faster for those candidates. Needs some further investigation.
     *
