 ...
 */
public class Lehman_TillSimpleBig extends FactorAlgorithmBase {
	...
	public Lehman_TillSimpleBig(float tDivLimitMultiplier) {
	...
	public long findSingleFactor(long N) {
		...
		// make it odd
		final int twoA = Math.max(((kLimit >> 6) - 1), 0) | 1;
		...
		long a = (long) (sqrt4N * sqrt[kSmall-2] + ROUND_UP_DOUBLE) | 1;
		//		final BigInteger aBig = BigInteger.valueOf(a);
		//		final long fourKN = kSmall * fourN;
		//		final long eightKN = fourKN << 1;
		//		final BigInteger fourKNBig = BigInteger.valueOf(fourKN);
		//		final BigInteger testBig = aBig.multiply(aBig).subtract(fourKNBig);
		//		long test2 = testBig.longValue();
		// 2. continue main loop for larger even k, where we can have only 4 a values per k
		for (int kEven = kSmall; kEven <= kLimit; kEven+=2) {
			final long a3 = (long) (a + a/(kEven - 1.5) + 2.5);
			final long aCeil = (long) (sqrt4N * sqrt[kEven] + ROUND_UP_DOUBLE);
			a = aCeil | 1;
			// a = a * sqrt(k+2)/sqrt(k)
			// a = a * sqrt(1 + 2/k)
			// a = a * (1+ 1/k)
		...
		for ( ; kOdd <= kLimit; kOdd += 2) {
			a = (long) (sqrt4N * sqrt[kOdd] + ROUND_UP_DOUBLE);
			a += (kOdd + N - a) & 3;
