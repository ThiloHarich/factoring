 ...
 */
public class Lehman_CustomKOrderTh3 extends FactorAlgorithm {
	private static final Logger LOG = Logger.getLogger(Lehman_CustomKOrderTh3.class);

	/** This is a constant that is below 1 for rounding up double values to long. */
	private static final double ROUND_UP_DOUBLE = 0.9999999665;

	private static final int K_MAX = 1<<17;

	private final TDiv63Inverse tdiv = new TDiv63Inverse(K_MAX*4);
	 ...
	 */
	public Lehman_CustomKOrderTh3(boolean doTDivFirst) {
	...
	public long findSingleFactor(long N) {
		...
		//		if ((factor = test(kTwoA, 2 * kLimit, sixthRootTerm, ks, sqrts, sqrtInvs)) > 1) return factor;
		int i = 0;
		for ( ; i < 2*kLimit; i++) {
			final int k = ks[i];
		...
		// finish Lehman loops
		//		if ((factor = test(kTwoA, kLimit, sixthRootTerm, ks1, sqrts1, sqrtInvs1)) > 1) {
		//			System.out.print("X");
		//			return factor;
		//		}
	...
	public static void main(String[] args) {
		...
		final Lehman_CustomKOrderTh3 lehman = new Lehman_CustomKOrderTh3(false);
