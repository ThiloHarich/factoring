 ...
 */
public class Lehman_TillSimple6 extends FactorAlgorithmBase {
	private static final Logger LOG = Logger.getLogger(Lehman_TillSimple6.class);
	...
	public Lehman_TillSimple6(float tDivLimitMultiplier) {
	...
	public long findSingleFactor(long N) {
		...
		// 2. continue main loop for larger even k, where we can have only 2 'a' values per k
		// here we first inspect k = 6*i then k = 6*1 + 2 and k = 6*1 + 4
		// since they much more likely have a solution x^2 - sqrt(k*n) = y^2
		//
		...
		for (int kEven = kSmall + 2; kEven <= kLimit; kEven += 6) {
			final long a = (long) (sqrt4N * sqrt[kEven] + ROUND_UP_DOUBLE) | 1;
			final long test = a*a - kEven * fourN;
			final long b = (long) Math.sqrt(test);
			if (b*b == test) {
		...
		//		for (int kEven = kSmall + 4; kEven <= kLimit; kEven += 6) {
		//			final long a = (long) (sqrt4N * sqrt[kEven] + ROUND_UP_DOUBLE) | 1;
		//			final long test = a*a - kEven * fourN;
		//			final long b = (long) Math.sqrt(test);
		//			if (b*b == test) {
