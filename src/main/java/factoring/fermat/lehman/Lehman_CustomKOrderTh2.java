 ...
 */
public class Lehman_CustomKOrderTh2 extends FactorAlgorithm {
	private static final Logger LOG = Logger.getLogger(Lehman_CustomKOrderTh2.class);
	...
	private final double[] sqrts315;
	private final double[] sqrtInvs315;
	private final int[] ks315;
	private final double[] sqrts630;
	private final double[] sqrtInvs630;
	private final int[] ks630;
	 ...
	 */
	public Lehman_CustomKOrderTh2(boolean doTDivFirst) {
		this.doTDivFirst = doTDivFirst;
		// arrange k in different arrays
		sqrts315 = new double[K_MAX+1];
		sqrtInvs315 = new double[K_MAX+1];
		ks315 = new int [K_MAX+1];
		sqrts630 = new double[K_MAX+1];
		sqrtInvs630 = new double[K_MAX+1];
		ks630 = new int [K_MAX+1];
		sqrts1 = new double[K_MAX+1];
		sqrtInvs1 = new double[K_MAX+1];
		ks1 = new int [K_MAX+1];

		for (int pos = 0; pos < K_MAX; pos ++) {
			ks630[pos] = 315 * (2*pos+2);
			sqrts630[pos] = Math.sqrt(ks630[pos]);
			sqrtInvs630[pos] = 1 / sqrts630[pos];
			ks315[pos] = 315 * (pos+1);
			//			ks315[pos] = 315 * (2*pos+1);
			sqrts315[pos] = Math.sqrt(ks315[pos]);
			sqrtInvs315[pos] = 1 / sqrts315[pos];
			ks1[pos] = pos+1;
	...
	public long findSingleFactor(long N) {
		...
		//		if ((factor = test(kTwoA, 4 * 315 * kLimit, sixthRootTerm, ks630, sqrts630, sqrtInvs630)) > 1) return factor;
		if ((factor = test(kTwoA, kLimit, sixthRootTerm, ks315, sqrts315, sqrtInvs315)) > 1) return factor;
	...
	public static void main(String[] args) {
		...
		final Lehman_CustomKOrderTh2 lehman = new Lehman_CustomKOrderTh2(false);
