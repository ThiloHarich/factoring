 ...
 */
public class Lehman_TillSimple9 extends FactorAlgorithmBase {
	private static final Logger LOG = Logger.getLogger(Lehman_TillSimple9.class);
	...
	public Lehman_TillSimple9(float tDivLimitMultiplier) {
	...
	public long findSingleFactor(long N) {
		...
		// make it odd
		int twoA = Math.max(((kLimit >> 4) - 1), 0) | 1;
		...
		for (int kEven = kSmall ; kEven <= kLimit; kEven += 3) {
			final long a = (long) (sqrt4N * sqrt[kEven] + ROUND_UP_DOUBLE);
		...
		for (int kEven = kSmall + 1; kEven <= kLimit; kEven += 3) {
			final long a = (long) (sqrt4N * sqrt[kEven] + ROUND_UP_DOUBLE);
			final long test = a*a - kEven * fourN;
			final long b = (long) Math.sqrt(test);
			if (b*b == test) {
		...
		// 3. continue main loop for larger k, where we can have only 2 'a' values per k
		// here we only inspect k = 6*i + 3 since they much more likely have a solution x^2 - sqrt(k*n) = y^2
		for (int kEven = kSmall + 2; kEven <= kLimit; kEven += 3) {
			// make a even
			final long a = (long) (sqrt4N * sqrt[kEven] + ROUND_UP_DOUBLE);
			final long test = a*a - kEven * fourN;
			final long b = (long) Math.sqrt(test);
